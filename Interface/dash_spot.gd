...
func _on_EVENTS_show_Dspot(starting_position: Vector2) -> void :
    ...
    for n in 3:
        var tempo_position = calcul_decal_position(n + 1)
        var tile_check: = node_tile_map_layer_wall.local_to_map(tempo_position)
        if node_tile_map_layer_wall.get_cell_source_id(tile_check) != -1:
            visible = false
            return
